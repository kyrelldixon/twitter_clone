defmodule TwitterApiWeb.V1.RelationshipController do
  ...
  def create(conn, %{"relationship" => relationship_params}) do
    with {:ok, %Relationship{} = relationship} <- Accounts.create_relationship(relationship_params) do
      conn
      |> put_status(:created)
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %Relationship{}} <- Accounts.delete_relationship(relationship) do
